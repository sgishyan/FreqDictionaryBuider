...
public class PlumberHugePressureModel {
    ...
    public PlumberHugePressureModel(int[][] array, int h, int w, int containersNumber, ArrayList<Pair<Integer, Integer>> containers, ArrayList<Pair<Integer, Integer>> valves)
	{
		...
		for(int i=0;i<height;i++)
			for(int j=0;j<width;j++)
			{
				...
				switch(tube)
				{
                    ...
                    case 8:
                        gameBoard[i][j]=new LargeEnd(rotation);
                        break;
				}
    ...
    public ArrayList<ArrayList<WaterPath>> shortestPath (int containerIndex, int minSize) {
       ...
       // System.out.println("SINGLE SOLUTION" + allSolutions.size()) ;
        ...
        //ToDo checking multi usage(with count)
        int multiUseCount = 0;
        ...
        for (int r = 0; r < rotationsLimit; r++) {
            ...
            switch (weCameFrom) {
                ...
                case NO_WAY:
                    curX = oldX;
                    curY = oldY;
                    if (!usageArray[curX][curY]) gameBoard[curX][curY].rotate();
...
//            System.out.println("New_TUBETYPE : " + gameBoard[curX][curY].getTubesType());
            ...
            if (gameBoard[curX][curY].getTubeType() == TubeType.END && gameBoard[curX][curY].getDirection(weCameFrom) == Direction.FINISH) {

                //System.out.println("End Tube");
