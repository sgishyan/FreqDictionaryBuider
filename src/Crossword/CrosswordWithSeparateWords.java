 ...
 */
public class CrosswordWithSeparateWords {
    private char[][] table;
    private boolean[][] tableBlocks;
    ...
    private ArrayList<LocationSide> separationSide;
    private int wordsCount;
    private int separations;
    private boolean hasSpecialCells = false;
    private Random random;
    public CrosswordWithSeparateWords(ArrayList<String> dictionary, ArrayList<String> crWords, String word , Difficulty difficulty) {
        ...
        separationSide = new ArrayList<>();
        separationSide.add(LocationSide.EAST);
        separationSide.add(LocationSide.NORTH);
        separationSide.add(LocationSide.WEST);
        separationSide.add(LocationSide.SOUTH);

        Collections.shuffle(separationSide);
        ...
        switch (word.length()) {
            ...
            case 7:
                ...
                break;
            case 8:
                tableSize = 11 + random.nextInt(2);
                wordsCount = 12;
                break;

        }

        Collections.sort(crWords, new Comparator<String>() {
            @Override
        ...
        tableBlocks = new boolean[tableSize][tableSize];
        for (int i = 0; i < tableSize; i++) {
            for (int j = 0; j < tableSize; j++) {
                table[i][j] = 0;
                tableBlocks[i][j] = false;
            }
        }
        ...
        //Placing separate words
        for (int i = 0; i < separations; i++ ) {
            int index = random.nextInt(crWords.size() / 2);
            if (usedWords.contains(crWords.get(index))) {
                i--;
                continue;
            }
            boolean canAdd = placeWordSeparate(crWords.get(index), separationSide.get(0));
            if (canAdd) {
                usedWords.add(crWords.get(index));
                separationSide.remove(0);
            }else {
                continue;
            }
    ...
    private void markInvisableCells() {
        for (int i = 1; i < tableSize - 1; i++) {
            for (int j = 1; j < tableSize - 1; j++) {


                if (table[i][j] == 0 && table[i - 1][j] != 0 && table[i + 1][j] != 0 &&
                        table[i][j + 1] != 0 && table[i][j - 1] != 0) {
                    table[i][j] = '+';
    ...
    public boolean hasInvisablCells() {
        return hasSpecialCells;
    }

    private boolean placeWordSeparate(String word, LocationSide separationSide) {

        int x = 0;
        int y = 0;
        int startShift = 0;
        int totalPlaces= tableSize - word.length();
        boolean isHorizontal = false;

        switch (separationSide) {
            case EAST:

                if (tableBlocks[0][tableSize - 1] == true) {
                    startShift = 2;
                    totalPlaces-= 2;
                }else
                if(tableBlocks[0][tableSize - 2]){
                    startShift = 1;
                    totalPlaces--;
                }

                if (tableBlocks[tableSize - 1][tableSize - 1] == true) {
                    totalPlaces -= 2;
                }else
                if (tableBlocks[tableSize - 1][tableSize - 2] == true) {
                    totalPlaces -= 1;
                }
                ...
                if (totalPlaces == 0) {
                    ...
                    x = random.nextInt(totalPlaces) + startShift;
                    y = tableSize - 1;
                }
            ...
            case WEST:
                ...
                if (tableBlocks[tableSize - 1][0] == true) {
                    totalPlaces -= 2;
                }else
                if (tableBlocks[tableSize - 1][1] == true) {
                    totalPlaces -= 1;
                }
            ...
            case NORTH:
                ...
                if (tableBlocks[1][tableSize - 1] == true) {
                    totalPlaces -= 1;
                }
                ...
                if (totalPlaces == 0) {
                    ...
                    x = 0;
                    y = random.nextInt(totalPlaces) + startShift;;
                }
            ...
            case SOUTH:
                ...
                if (tableBlocks[tableSize - 2][tableSize - 1] == true) {
                    totalPlaces -= 1;
                }
        ...
        FixedWord fixedWord = new FixedWord(x,y,isHorizontal, word);
        placedWords.add(fixedWord);
        addToTable(fixedWord, true);
        return true;
    }
...
//    public boolean placeWordSeparate(String word, LocationSide locationSide) {
//        ArrayList<FixedWord> availablePlaces = new ArrayList<>();
//        for (int i = 0; i < tableSize; i++) {
//            for (int j = 0; j < tableSize; j++) {
//                availablePlaces.add(new FixedWord(i, j, false, word));
...
//        Collections.shuffle(availablePlaces);
//        for (FixedWord fixedWord : availablePlaces) {
//            boolean isPossible = tryToPlaceWordSeparate(fixedWord);
//            if (isPossible) {
//                placedWords.add(fixedWord);
...
//    }
    ...
    private void addToTable(FixedWord fixedWord, boolean isSeparateWord) {
        int dx = 0;
        int dy = 0;
    ...
    private boolean tryToPlaceWordSeparate(FixedWord fixedWord) {
        int dx = 0;
        int dy = 0;
        int x = fixedWord.getStartX();
        int y = fixedWord.getStartY();
        ...
        //If first word, no crosses are allowed
    ...
    private boolean tryToPlaceWord(FixedWord fixedWord) {
       //  System.out.println("-----------------------------------------");
        ...
        int x = fixedWord.getStartX();
        int y = fixedWord.getStartY();
       // System.out.println(x + " " + y + " " + fixedWord.getWord());
        int crossesCount = 0;
        boolean[][] magicCells = new boolean[tableSize][tableSize];
        if (fixedWord.isHorizontal()) {
            dy = 1;
        ...
        if (crossesCount == 0 && placedWords.size() > separations) {
            // System.out.println("No crosses found : " +  fixedWord.getStartY() + " " + fixedWord.getStartY() + " " + fixedWord.isHorizontal() );
            return false;
        } else if (crossesCount == 0 && placedWords.size() == 0) {
            //System.out.println("First word");
    ...
    public static void main(String[] args) {
        ...
        words.add("лот");
        words.add("тол");
        CrosswordWithSeparateWords c = new CrosswordWithSeparateWords(null, words, "стекло", Difficulty.HARD);
       // c.print();
//        while(true) {
//            Crossword.CrosswordWithSeparateWords.CrosswordWithSeparateWords crossword = new Crossword.CrosswordWithSeparateWords.CrosswordWithSeparateWords(words, words, "восток");
//            crossword.print();
