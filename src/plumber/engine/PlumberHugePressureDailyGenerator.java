 ...
 */
public class PlumberHugePressureDailyGenerator {

    private Random rand;
    private PlumberHugePressureModel model;
    int HEIGHT = 10;
    int WIDTH = 8;
    Map<Integer,Integer> desiredLevels = new HashMap<>();
...
//    final int TOTAL_EMPTY = 10;
    int [][] level;
    public PlumberHugePressureDailyGenerator(int count, int minComplexity, int maxComplexity) {
        ...
        int step = 1;
        count = 6;
        int levelStartNumber = (int) (30);
        int levelNumber = 251;
        int[] levelGenerated = new int[count];
        //shifting levels for larger size
        int shifter = 0;
        double multiplier = 3;


        //ToDo: Must be part of custom arrays
        int containersCount = 4;
        for(int i = 0; i < count; i++) {

            int dif = difficulties[ i + 350];
            if (desiredLevels.containsKey(dif)) {
                desiredLevels.put(dif, desiredLevels.get(dif) + 1);
//                System.out.println(dif);
            }else {
                desiredLevels.put(dif,  1);
            }
            levelGenerated[i] = dif;
        ...
        while(levelsGenerated < count) {
            ...
            //TOTAL_ANGLES = anglesCountPerPackage[((int) ((levelStartNumber + shifter + levelsGenerated * multiplier) / 16))] + rand.nextInt(anglesCountPerPackageRandComponent[((int) ((levelStartNumber + levelsGenerated * multiplier + shifter) / 16))]) ;

           // TOTAL_ANGLES = 10 + rand.nextInt(4);
            TOTAL_ANGLES = 15 + rand.nextInt(20);
            //Searching container: 1

//            if (levelStartNumber + shifter + levelsGenerated * multiplier <= 256) {
//                WIDTH = 6;
//                HEIGHT = 8;
//                TOTAL = 48;
//            }

            TOTAL_TUBES = TOTAL - TOTAL_ANGLES - 2 * containersCount;
            PlumberHugePressureModel model = generateLevel(containersCount);
...
//                continue;
//            }
            ArrayList<ArrayList<WaterPath>> solution = model.findAllShortestPaths(4);
            ...
            if (solution != null) {
                ...
                //ToDo : check position modifier
                double levelPositionValue_modifier = 0.85;
                if (levelNumber > 16) {
                    levelPositionValue_modifier = 1;
                }
                levelPositionValue_modifier = 1;
                difficulty = (int)(levelPositionValue_modifier * (TOTAL_ANGLES + 2 * allSolutionsLength) / 2) ;
                //difficulty = (int) (allSolutionsLength * 1.1);
                //System.out.println(difficulty);
//                if (difficulty > 70) {
//                    System.out.println(solution.size() + " " + difficulty);
//                }
             //   System.out.println("Desired " + (difficulties[((int) (levelStartNumber + shifter + levelsGenerated * multiplier))]));
                System.out.println("Difficulty " +difficulty + "  TOTAL ANGLES " + TOTAL_ANGLES);
             //   //Todo open after 1 levels
             //   if (Math.abs(difficulty - difficulties[((int) (levelStartNumber + shifter + levelsGenerated * multiplier))]) > eps ) {
              //      continue;
               // }
                for(int i = 0; i < count; i++) {
                    if (Math.abs(difficulty - levelGenerated[i]) < eps) {
                        System.out.println("Found " + (levelsGenerated) +  " : "  + i + " dif" + difficulty);
                        writeToFile("/home/suren/Documents/temp/plumber/for_sale/extreme_pressure/" + (levelNumber + i) + ".txt", solution);
                        levelsGenerated++;
                        levelGenerated[i] = -1;
                        break;
...
//                    continue;
//                }
                System.out.println("Level " + (levelNumber) + "  " + solution.size() + " Diff = " + difficulty);
                for (int i = 0 ; i < solution.size(); i++) {
                    System.out.println(solution.get(i).size());
                }
                System.out.println(model.toString());
                writeToFile("D:\\Documents\\plumber_levels\\high_presure\\" + (levelNumber) + ".txt", solution);
    ...
    private PlumberHugePressureModel generateLevel(int containersCount) {
       // System.out.println("=============================================================");
        int totalTubes = HEIGHT * WIDTH;
        ArrayList<Pair<Integer, Integer>> containers = new ArrayList<>();
...
//        }
        ...
        //Choosing best place for Pressure device
        int pressureX = rand.nextInt(HEIGHT - 5) + 2;
        int pressureY = rand.nextInt(WIDTH - 5) + 2;
       // System.out.println("Press " + pressureX + " " + pressureY);
        //Filling all cells, but the pressure device
        for (int i = 0; i < HEIGHT; i++)
            for (int j = 0; j < WIDTH; j++) {
                if (i >= pressureX && i - pressureX < 2 && j >= pressureY && j - pressureY < 2) {
...
//        mobiloids.Pair<Integer, Integer> valve = cells.get(0);
        ...
        level[pressureX][pressureY] = 80;
        level[pressureX][pressureY + 1] = 81;
        level[pressureX + 1][pressureY] = 83;
        level[pressureX + 1][pressureY + 1] = 82;
        containers.add(new Pair<>(pressureX, pressureY));
        containers.add(new Pair<>(pressureX, pressureY + 1));
        containers.add(new Pair<>(pressureX + 1, pressureY));
        containers.add(new Pair<>(pressureX + 1, pressureY + 1));
...
//        }
        ...
        for (int i = 0; i < containersCount; i++) {
            Pair<Integer, Integer> end = cells.get(i);
            //Adding container with no rotation
            level[end.getKey()][end.getValue()] = 50 + rand.nextInt(4);

            valves.add(cells.get(i));
            // System.out.println("Added container " + end.getKey() + "  " +  end.getValue());
        }
        //Adding valves with some rotation
...
//        level[HEIGHT - 1][WIDTH - 1] = 60 + rand.nextInt(4);
      ...
      //  System.out.println(angles + " " + doubleAngles + " " + tubes + " " + doubleTubes);
        int cellsIndex = containersCount;
        //Placing angle tubes
        ...
        model = new PlumberHugePressureModel(level, HEIGHT, WIDTH, containersCount, containers, valves);
//        System.out.println("-------------------------------------------------------------");
//        for (int i= 0; i< HEIGHT; i++) {
//            for(int j = 0; j< WIDTH; j++) {
...
//        maxComplexity = sc.nextInt();
        ...
        PlumberHugePressureDailyGenerator generator = new PlumberHugePressureDailyGenerator(365, 0, 100);
