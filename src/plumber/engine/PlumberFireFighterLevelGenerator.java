 ...
 */
public class PlumberFireFighterLevelGenerator {

    private Random rand;
    private PlumberFireFighterModel model;
    int HEIGHT = 4;
    int WIDTH = 6;
    ...
    int TOTAL_EMPTY = 0;
    int difficulty = 0;
    private double TIME_MODIFIER = 0.6;
//
//    final int TOTAL = 24;
...
//    final int TOTAL_EMPTY = 10;
    int [][] level;
    public PlumberFireFighterLevelGenerator(int count, int minComplexity, int maxComplexity) {
        ...
        double[] timeDeltaBy8 =  {1.5, 1.2, 1, 0.9, 0.85, 0.8, 0.75, 0.7,
                                    0.65, 0.6, 0.55, 0.5, 0.5, 0.5, 0.45, 0.4};
...
//                System.out.println(dif);
            ...
            System.out.println(i + "  " + dif + " " + houses[i + 32] );
        }

        while(levelsGenerated < count) {

            containersCount = houses[levelNumber + levelsGenerated - 1];
            containersCount = 2;
          ...
          ///      HEIGHT = 8;
           ...
           // }
            ...
            TOTAL_TUBES = TOTAL - TOTAL_ANGLES - containersCount - 1;
            PlumberFireFighterModel model = generateLevel(containersCount);
            ...
            if (solution != null) {
               ...
               // System.out.println(allSolutionsLength);
                double levelPositionValue_modifier = 0.85;
                if (levelNumber + levelsGenerated > 110) {
                    levelPositionValue_modifier = 0.7;
                }else
                if (levelNumber + levelsGenerated > 80) {
                    levelPositionValue_modifier = 0.7;
                }else
                if (levelNumber + levelsGenerated > 60) {
                    levelPositionValue_modifier = 0.75;
                }else
                if (levelNumber + levelsGenerated > 40) {
                    levelPositionValue_modifier = 0.8;
                }

                difficulty = (int) ((TOTAL_ANGLES + 2 * levelPositionValue_modifier * allSolutionsLength) / 2);
...
//                        System.out.println("Found " + (levelsGenerated) +  " : "  + i + " dif" + difficulty + " " + containersCount);
//                        writeToFile("/home/suren/Documents/temp/plumber/update/fire/" + (levelNumber + i) + ".txt", solution);
//                        levelsGenerated++;
//                        levelGenerated[i] = -1;
//                        break;
//
...
////                }
                ...
                writeToFile("/home/suren/Documents/temp/plumber/ingame/fire/" + (levelNumber + levelsGenerated) + ".txt", solution);
    ...
    private PlumberFireFighterModel generateLevel(int containersCount) {
...
//        level[HEIGHT - 1][WIDTH - 1] = 60 + rand.nextInt(4);
      ...
      //  System.out.println(angles + " " + doubleAngles + " " + tubes + " " + doubleTubes);
        int cellsIndex = 1 + containersCount;
        //Placing angle tubes
        ...
        model = new PlumberFireFighterModel(level, HEIGHT, WIDTH, containersCount);
...
//        }
    ...
    public void writeToFile(String filename, ArrayList<ArrayList<WaterPath>> solution) {
        ...
        try {
            ...
            output.write("Fire" +"\n");
            output.write((int)(difficulty * TIME_MODIFIER)+"\n");
            output.write(HEIGHT +"\n");
            output.write(WIDTH +"\n");
            for (int i = 0; i < HEIGHT; i++) {
                for (int j = 0; j < WIDTH; j++) {
...
//        maxComplexity = sc.nextInt();
        ...
        PlumberFireFighterLevelGenerator generator = new PlumberFireFighterLevelGenerator(365, 0, 100);
