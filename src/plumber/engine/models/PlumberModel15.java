...
public class PlumberModel15 extends Model {
    ...
    private ArrayList<WaterPathSegment> singleSolution;
    private int[][] solutionBoard;

    private Colors color = Colors.ALL_COLORS;
    private Pair<Integer, Integer> valve;

    public PlumberModel15(int[][] array, ArrayList<SolutionSegment> solution, int h, int w, ArrayList<Point> arrayOfCoords) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                switch (tube) {
                    ...
                    case 6:
                        gameBoard[i][j] = new EndTube15(rotation);
                        if (colorCode != 0) {
                            gameBoard[i][j].setColor(getColorFromCode(colorCode));
                        }else {
    ...
    public PlumberModel15(int[][] board, int heigth, int width) {
    ...
    public int[][] shortestPath () {
        ...
        singleSolution = solutions.get(0);
    ...
    public ArrayList<WaterPathSegment> getSingleSolution() {
      ...
      //      System.out.println("Rotations " + gameBoard[curX][curY].getUniquePositionsCount());
        for (int r = 0; r < rotationsLimit; r++) {
            ...
            if (gameBoard[curX][curY].getTubeType() == TubeType.END) {
                ...
                if (gameBoard[curX][curY].getDirection(weCameFrom) != Direction.FINISH) {
                    // System.out.println("End tube wrong side");
                    return ;
                }
                if (gameBoard[curX][curY].getColor() == pathColor || gameBoard[curX][curY].getColor() == Colors.ALL_COLORS) {
                    ...
                    for (int i = 0; i < height; i++) {
                        for (int j = 0; j < width; j++) {
                            solutionBoard[i][j] = gameBoard[i][j].getTubesTypeIndex() * 10 +  gameBoard[i][j].getRotation();
                        }
                    }
                }else {
                    return;
    ...
    @Override
    public boolean makePath(int startX, int startY, Colors startColor) {
...
//                    + " type " + gameBoard[curX][curY].getTubesType() + "rotation = " + gameBoard[curX][curY].getRotation());
    ...
    boolean checkForPortal(int curX, int curY, boolean alreadyUsesPortal) {
      ...
      //  System.out.println(curX + " " + curY);
        if (curX < 0 || curX >= height || curY < 0 || curY >= width)
            return false;
   //     System.out.println(gameBoard[curX][curY].getTubeType());
...
//        Log.i("Hint", totalRemoved + "");
    ...
    public int makeValvePath() {
        return makePath15(valveX, valveY, gameBoard[valveX][valveY].getColor());

    }
    public int makePath15(int startX, int startY, Colors startColor) {
        ...
        while (true) {
            ...
            if (gameBoard[curX][curY].getTubeType() == TubeType.END) {
               ...
               // if (isThereColorSwitcherElement) {
                    if (startColor != gameBoard[curX][curY].getColor()) {
                        //System.out.println("End tube wrong color");
                        return 0;
                    }

                //}
                if (gameBoard[curX][curY].getDirection(convertDirection(moveDirection)) != Direction.FINISH) {
                   // System.out.println("End tube wrong side");
                    return 0;
                }
                //System.out.println("!!!  End tube right side  !!!");
