 ...
 */
public class PlumberMultiValveGeneratorTemplate {

    private Random rand;
    private PlumberMultiValveModel model;
    int HEIGHT = 4;
    int WIDTH = 6;
...
//    final int TOTAL_EMPTY = 10;
    int [][] level;
    int [][] template;
    public PlumberMultiValveGeneratorTemplate(int count, int minComplexity, int maxComplexity) throws IOException {
        ...
        template = readFile("/home/suren/Documents/temp/plumber/templates/template.txt");
        System.out.println("template is read");
        int levelsGenerated = 0;
        int targetDifficulty = 10;
        int step = 1;
        int eps = 3;
        count = 1;
        int levelStartNumber = 11;
        int minSize = 10;
        while(levelsGenerated < count) {
            ...
            TOTAL_TUBES = TOTAL - TOTAL_ANGLES - 2;
             ...
             //  TOTAL_ANGLES = 25 + rand.nextInt(25);
              // TOTAL_ANGLES = 10 + rand.nextInt(15);
               TOTAL_ANGLES = 5 + rand.nextInt(5);
               ...
               PlumberMultiValveModel model = generateLevelWithTemplate(template, containersCount);
...
//            }
               ...
               ArrayList<ArrayList<WaterPath>> solution = model.findAllShortestPaths(minSize);
...
//                }
                   ...
                   for(int i = 0; i < count; i++) {
                       if (Math.abs(difficulty - targetDifficulty) < eps) {
                           System.out.println("Found " + (levelsGenerated) +  " : "  + i + " dif" + difficulty);
                           writeToFile("/home/suren/Documents/temp/plumber/template_levels/" + (levelStartNumber + levelsGenerated) + ".txt", solution);
                           levelsGenerated++;
                           break;

                       }
                   }
                /*   if (desiredLevels.containsKey(difficulty)) {
                       System.out.println("Found " + levelNumber++ + " dif" + difficulty);
...
//                }
               /* System.out.println("Level " + (levelStartNumber + levelsGenerated) + "  " + solution.size() + " Diff = " + difficulty);
                ...
                writeToFile("/home/suren/Documents/temp/plumber/soft_launch/multivalve/" + (levelStartNumber + levelsGenerated) + ".txt", solution);
    ...
    private PlumberMultiValveModel generateLevelWithTemplate(int [][] template, int containersCount) {
       // System.out.println("=============================================================");
...
//        }
        ...
        boolean hasValve = true;
        boolean hasContainer = true;
        ArrayList<Pair<Integer,Integer>> cells = new ArrayList<>(48);
        for (int i = 0; i < template.length; i++) {
            for (int j = 0; j < template[i].length; j++) {
                if (template[i][j] == 70 || template[i][j] == 100) {
                    // System.out.println(template[i][j]);
                    cells.add(new Pair<>(i, j));
                    randomTiles++;
                } else {
                    level[i][j] = template[i][j];
                }
                if (template[i][j] / 10 == 5) {
                    hasValve = false;
                }

                if (template[i][j] / 10 == 6) {
                    hasContainer = false;
                }
            }
        }
        System.out.println("randoms = " + randomTiles);
        if (hasContainer || hasValve) {
            System.out.println("ERROR: SHOULD NOT BE ANY CONTAINER OR VALVE");
            return null;
        }
...
//        }
        ...
        for (int i = 0; i < containersCount; i++) {
            Pair<Integer, Integer> end = cells.get(i + containersCount);
            //Adding container with no rotation
            level[end.getKey()][end.getValue()] = 50;
            valves.add(cells.get(i + containersCount));
            // System.out.println("Added container " + end.getKey() + "  " +  end.getValue());
        }
        //Adding valves with some rotation
...
//        level[HEIGHT - 1][WIDTH - 1] = 60 + rand.nextInt(4);
      ...
      //  System.out.println(angles + " " + doubleAngles + " " + tubes + " " + doubleTubes);
        int cellsIndex = 2 * containersCount;
        //Placing angle tubes
        ...
        model = new PlumberMultiValveModel(level, HEIGHT, WIDTH, containersCount, containers, valves);
...
//        }
    ...
    private static int[][] readFile(String path1) throws IOException {
...
//        maxComplexity = sc.nextInt();
        ...
        PlumberMultiValveGeneratorTemplate generator = new PlumberMultiValveGeneratorTemplate(365, 0, 100);
