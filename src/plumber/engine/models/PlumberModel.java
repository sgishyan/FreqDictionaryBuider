...
public class PlumberModel extends Model {
    ...
    public PlumberModel(int[][] array, ArrayList<SolutionSegment> solution, int h, int w, ArrayList<Point> arrayOfCoords) {
    ...
    public PlumberModel(int[][] board, int heigth, int width) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (gameBoard[i][j].getTubeType() == TubeType.VALVE) {
                    return makePath15(i, j, gameBoard[i][j].getColor());
                }
            }
        }
        return 0;
